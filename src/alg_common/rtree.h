...
public:
    ...
    bool KNN(const ELEMTYPE coord[NUMDIMS], const int k_value, std::vector<ELEMTYPEREAL>& result, ELEMTYPEREAL dis_limit = -1, t_resultCallback predicate = nullptr, void* a_context = nullptr);

	bool NearstWithDis(const ELEMTYPE coord[NUMDIMS], std::vector<ELEMTYPEREAL>& result, ELEMTYPEREAL dis_limit = -1, t_resultCallback predicate = nullptr, void* a_context = nullptr);
...
RTREE_TEMPLATE
bool RTREE_QUAL::NearstWithDis(const ELEMTYPE coord[NUMDIMS], std::vector<ELEMTYPEREAL>& result, ELEMTYPEREAL dis_limit, t_resultCallback predicate, void* a_context)
{
	int k_value = 1;
		...
		while (knnode_queue.size())
		{
			...
			if (knode.isLeaf())
			{
				...
				if (!predicate || predicate(branch->m_data, a_context))
				{
					...
					result.push_back(knode.m_distance);

				}
				if (result.size() == size_t(k_value)) return true;
			}
			else
			{
				break;
...
RTREE_TEMPLATE
    bool RTREE_QUAL::KNN(const ELEMTYPE coord[NUMDIMS], const int k_value, std::vector<ELEMTYPEREAL>& result, ELEMTYPEREAL dis_limit, t_resultCallback predicate, void* a_context)
{
    std::priority_queue<KNNode, std::vector<KNNode>, std::greater<KNNode> > knnode_queue;
    ...
    ELEMTYPEREAL RTREE_QUAL::CalcPointRectDis(const ELEMTYPE coord[NUMDIMS], const Rect* a_rect)
{
    ...
    for(int axis=0; axis<NUMDIMS; ++axis)
    {
		/*printf("RTREE_QUAL::CalcPointRectDis NUMDIMS = %d, axis = %d,coord[axis]=%d, 
		a_rect->m_min[axis] = %d,  a_rect->m_max[axis]=%d, distance = %d\n",
			NUMDIMS, axis, coord[axis], a_rect->m_min[axis], a_rect->m_max[axis], distance);*/
...
typedef RTree<int, int, 2, int> RTree2i;
typedef RTree<double, int, 2, double> RTree2d;

typedef RTree<float, float, 2, float> RTree2f;
