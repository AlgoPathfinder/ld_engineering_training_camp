 ...
 * @LastEditTime: 2023-08-18 00:01:40
 * @LastEditors: Ang.Lee.
 * @Description: 
 * 
 */
...
public:
	...
	int SetMapData(int w, int h, float r, uint8_t* map_ptr)
	{
		map_w = w;
		map_h = h;
		resolution = r;
		mapdata = new uint8_t[w * h];
		memcpy(mapdata, map_ptr, w * h);
		return 0;
	}
			...
			for (int i = - search_size; i <= search_size; i++)
			{
				for (int j = -search_size; j <= search_size; j++)
				{
					...
					if ((idx_y + j >= 0) && (idx_y + j < map_h) && (idx_x + i > 0) && (idx_x + i < map_w))
					{
						...
						if (mapdata[cur_index] < 80)
						{
							//cur_likelyhood = exp(1/(cur_dis_sq+1) * (1/log(mapdata[cur_index])));
							//cur_likelyhood = exp(1 /(cur_dis_sq + 1));
							
							cur_likelyhood = exp(-1 * 0.05 *cur_dis_sq);
